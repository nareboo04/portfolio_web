...
export async function GET() {
  try {
    ...
    const sql = isAdmin
      ...
      : "SELECT * FROM `certifications` WHERE `status` = 'public' ORDER BY `sort_order` ASC, `issue_date` DESC"
    const rows = await query<Certification>(sql)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await execute(
      `INSERT INTO \`certifications\` (\`title\`, \`issuer\`, \`issue_date\`, \`expiry_date\`, \`description\`, \`credential_url\`, \`pdf_url\`, \`image_url\`, \`status\`, \`sort_order\`)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, (SELECT COALESCE(MAX(c.sort_order), 0) + 1 FROM \`certifications\` c))`,
      [title, issuer, issue_date, expiry_date, description, credential_url, pdfUrl, imageUrl, status],
    )
